...
class BenchmarkDatasets(BaseVisualization):
    ...
    def __init__(self, datasets: List[BaseDataset], log = True):
        ...
        self.metrics = ["ROC-AUC" if detect_setting(dataset.train_dataset[1]) == "classification" else "Root Mean Squared Error" for dataset in datasets]
    ...
    def get_data(self):
        ...
        for metric, manager in zip(self.metrics, self.managers):
            ...
            if direction == "higher":
                best_row = model_database[model_database[metric] == model_database[metric].max()].iloc[0]
            elif direction == "lower":
                best_row = model_database[model_database[metric] == model_database[metric].min()].iloc[0]

            d[best_row["Model Name"]] = d[best_row["Model Name"]] + 1

        out = dict()
        out["model_names"] = self.model_names
        out["model_params"] = [str(mp).replace("\n", "<br>") for mn, mp in zip(self.model_names, self.model_params)]
    ...
    def get_performance(self):
        ...
        for manager in self.managers:
            ...
            for i, row in manager_data.iterrows():
                index = i
                out.loc[out['Dataset'] == dataset_name, row['Model Name']] = row[self.metrics_dict[dataset_name]]
        return out
...
class BenchmarkMolNet(BenchmarkDatasets):
    ...
    def __init__(self, datasets: List[str] = ['all'], file_path = None, mode = "default", 
        ...
        if "default" in self.mode or "small" in self.mode:
            ...
            assert False, "Invalid mode, should be either 'default', 'small' or 'geognn' with optional attribute small"
        ...
        self.dataset_names = datasets
        self.dfs = dfs
        self.metrics = {name: "ROC-AUC" if self.datasets_types[name] == "classification" else "Root Mean Squared Error" for name in self.dataset_names}
        self.model_names = []
        self.model_params = []
    ...
    def run(self, models: Union[BaseModel, List[BaseModel]]):
        ...
        for model_ in tqdm(models):
            ...
            for name, df in tqdm(self.dfs.items()):
                ...
                self.model_database = self.model_database.append({"Model Name": model.name,
                    "Model Parameters": oce.pretty_params_str(model),
                    "Evaluation": self.eval,
                    "Dataset": name, "Metric": np.mean(l), "Full Metrics List": np.array(l)}, ignore_index=True)
